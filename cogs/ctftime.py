...
class CtfTime(commands.Cog):
    ...
    async def ctftime(self, ctx):
        if ctx.invoked_subcommand is None:
            ...
            await ctx.send(
                f"Current ctftime commands are: {', '.join(ctftime_commands)}"
            )
    ...
    async def current(self, ctx):
        ...
        for ctf in ctfs.find():
            if (
                ...
                embed.add_field(
                    name="Timeframe", value=start + " -> " + end, inline=True
                )
    ...
    async def upcoming(self, ctx, amount=None):
        ...
        for ctf in range(0, int(amount)):
            ...
            (ctf_start, ctf_end) = (
                upcoming_data[ctf]["start"].replace("T", " ").split("+", 1)[0] + " UTC",
                upcoming_data[ctf]["finish"].replace("T", " ").split("+", 1)[0]
                + " UTC",
            )
            (ctf_start, ctf_end) = (
                re.sub(":00 ", " ", ctf_start),
            ...
            embed.add_field(
                name="Timeframe", value=(ctf_start + " -> ") + ctf_end, inline=True
            )
            await ctx.channel.send(embed=embed)
    ...
    async def top(self, ctx, year=None):
        ...
        if r.status_code != 200:
            await ctx.send(
                'Error retrieving data, please report this with `>report "what happened"`'
            )
        else:
            try:
